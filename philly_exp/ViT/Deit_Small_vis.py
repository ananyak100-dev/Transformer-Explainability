...
#    '/home/t-akarthik/epoch14_checkpoint.pth.tar', map_location="cpu")
#   print(state_dict)
#   state_dict = state_dict['state_dict']
#   # remove `linear.` and 'module.' prefix
#   state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
#   self.model.load_state_dict(state_dict, strict=True).cuda()
#   self.model.eval()
 ...
 #  cam = self.linear_classifier.relprop(cam, **kwargs)
 #  cam = cam.unsqueeze(1)
 ...
 #  # print(kwargs)
 #  # print("conservation 1", cam.sum())
 #  cam = self.linear_classifier.relprop(cam, **kwargs) # used to be self.head
 #  cam = cam.unsqueeze(1)
 #  # cam = self.model.pool.relprop(cam, **kwargs) # not using pool
 #  cam = self.model.norm.relprop(cam, **kwargs)
 #  for blk in reversed(self.model.blocks):
 #   cam = blk.relprop(cam, **kwargs)
 #
...
for subdir in selected:
 subdir_save_folder = '/home/t-akarthik/Desktop/DeiT_Small/Compare/' + method + '/' + CLS2IDX[subdir]
...
for i, (input, target) in enumerate(val_loader, 0):
   if target in selected:
      ...
      with torch.no_grad():
       output = full_model.forward(input).cuda() # non_blocking=True?
...
for subdir in selected:
 ...
 print(len(group2))
 print(len(group3))
 visualize_all(group1, group2, group3, subdir)
